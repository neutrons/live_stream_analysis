...
# Generated by scripts/live_comparison/diamond_reference.py.
DEFAULT_REFLECTIONS_CSV = Path(__file__).parent / "live_comparison" / "diamond_reflections.csv"


def parse_reflections_csv(
    path: Path,
    min_rel_intensity: float,
    x_min: float | None,
    x_max: float | None,
) -> list[tuple[str, float]]:
    """Return (hkl, Q) for reference reflections inside the plotted range."""
    reflections: list[tuple[str, float]] = []
    with path.open("r", encoding="utf-8", newline="") as handle:
        reader = csv.DictReader(handle)
        required_columns = {"hkl", "Q (1/A)", "rel intensity"}
        if reader.fieldnames is None or not required_columns.issubset(set(reader.fieldnames)):
            raise ValueError("Reflections CSV must include columns: 'hkl', 'Q (1/A)', 'rel intensity'")
        for row in reader:
            q_value = float(row["Q (1/A)"])
            if float(row["rel intensity"]) < min_rel_intensity:
                continue
            if (x_min is not None and q_value < x_min) or (x_max is not None and q_value > x_max):
                continue
            reflections.append((row["hkl"], q_value))
    return reflections


def _draw_reflections(axis: plt.Axes, reflections: list[tuple[str, float]], annotate: bool) -> None:
    """Mark expected reflection positions as vertical lines behind the data."""
    for index, (_hkl, q_value) in enumerate(reflections):
        axis.axvline(
            q_value,
            color="0.55",
            linestyle="--",
            linewidth=0.8,
            zorder=0,
            label="expected reflections" if index == 0 else None,
        )
    if not annotate:
        return
    # Anchor labels to the axes, not the data, so they stay put when the view is zoomed.
    for hkl, q_value in reflections:
        axis.annotate(
            hkl,
            xy=(q_value, 1.0),
...
def _render_subplot_mode(
    ...
    for index, (label, q_values, intensity, error) in enumerate(series):
        ...
        if reflections:
            _draw_reflections(intensity_axis, reflections, args.annotate_reflections)
...
def _render_overlay_mode(
    ...
    intensity_axis.set_ylabel("I(Q)")
    intensity_axis.grid(True, alpha=0.3)
    if len(series) > 1 or reflections:
        intensity_axis.legend(loc="best")
    if relative_axis is not None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--with-diamond-reflections",
        action="store_true",
        help="Mark the expected diamond powder reflections (scripts/live_comparison/diamond_reflections.csv).",
    )
    parser.add_argument(
        "--reflections-csv",
        default=None,
        help="Mark reflections from this CSV instead of the diamond table. Implies the overlay.",
    )
    parser.add_argument(
        "--reflection-min-intensity",
        type=float,
        default=0.5,
        help="Skip reference reflections weaker than this relative intensity (default 0.5).",
    )
    parser.add_argument(
        "--annotate-reflections",
        action="store_true",
        help="Label each reflection marker with its hkl.",
    )
    return parser
...
def main() -> None:
    ...
    reflections: list[tuple[str, float]] = []
    if args.with_diamond_reflections or args.reflections_csv:
        reflections_path = Path(args.reflections_csv) if args.reflections_csv else DEFAULT_REFLECTIONS_CSV
        if not reflections_path.exists():
            raise ValueError(
                f"Reflections CSV not found: {reflections_path}. "
                "Generate it with scripts/live_comparison/diamond_reference.py"
            )
        reflections = parse_reflections_csv(
            reflections_path,
            args.reflection_min_intensity,
            args.x_min,
            args.x_max,
        )
        print(f"Marking {len(reflections)} reflections from {reflections_path.name}")

    if args.mode == "subplots" and len(series) > 1:
        figure = _render_subplot_mode(args, series, reflections)
    else:
        figure = _render_overlay_mode(args, series, reflections)
